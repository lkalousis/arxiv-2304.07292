...
To assess the advantages of the DFT approach towards gain calibration, we analyzed the same R7081 data set using the numerical method presented in ref.~\cite{dossi}.   
...
\item Second, the DFT approach gives a slightly better fit, since the $\chi^2$/NDOF is systematically smaller. 
Especially, one can see that as the Poisson mean $\mu$ increases the $\chi^2$/NDOF for the numerical integration method increases as well. 
...
On the other hand, DFT only took two seconds to analyze these six histograms.  
\end{enumerate}
